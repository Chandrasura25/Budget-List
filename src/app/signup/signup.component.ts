...
export class SignupComponent implements OnInit {
  ...
  creategoods() {
    let goodExist = this.Budget[this.id].Item.findIndex(
      (x: any) => x.name_good === this.name_good
    );
    if (goodExist == -1) {
      ...
      this.total_amount = this.Budget[this.id].total_amount;
      this.result = this.Budget[this.id].Item.reduce(
        (accumulator: any, obj: any) => {
          return accumulator + obj.totalquantity;
        },
        0
      ...
      this.message =
        'Goods Added! Click the blue tag to view your list of Goods';
      this.amount_goods = '';
      this.quantity = '';
      this.name_good = '';
      this.percent = 0.3 * this.total_amount;
